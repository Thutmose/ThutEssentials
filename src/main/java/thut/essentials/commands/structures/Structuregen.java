...
public class Structuregen
{
    ...
    private static SuggestionProvider<CommandSource> SUGGEST_NAMES = (ctx, sb) ->
    {
        ...
        for (final Entry<RegistryKey<Structure<?>>, Structure<?>> loc : ForgeRegistries.STRUCTURE_FEATURES.getEntries())
            opts.add(loc.getKey().getLocation().toString());
            ...
            final boolean doStructures)
    {
        ...
        // worldIn.getStructureTemplateManager();
        //
        // final ChunkGenerator generator =
        // worldIn.getChunkProvider().generator;
        // final BiomeManager bman = worldIn.getBiomeManager();
        //
        // int s = 0;
        // source.sendFeedback(new StringTextComponent("Initializing Chunks " +
        // s++), false);
        ...
        // s++), false);
        //
        // final MutableBoundingBox box =
        // MutableBoundingBox.getNewBoundingBox();
        // box.minY = minY;
        // box.maxY = maxY;
        //
        // final LazyOptional<IHasStructures> structs =
        // worldIn.getCapability(WorldStructures.CAPABILITY);
        //
        ...
        //
        // for (final Entry<Heightmap.Type, Heightmap> entry :
        // c.getHeightmaps())
        // if (ChunkStatus.FULL.getHeightMaps().contains(entry.getKey()))
        // o.getHeightmap(entry.getKey())
        // .setDataArray(entry.getValue().getDataArray());
        //
        // o.setLight(c.hasLight());
        // o.markDirty();
        // }
        // source.sendFeedback(new StringTextComponent("Regenerating Features
        // and Structures"), false);
        // for (final IChunk c : primers)
        // {
        ...
        // final long seed = random.setDecorationSeed(worldRegion.getSeed(), k,
        // l);
        // for (final GenerationStage.Decoration stage :
        // GenerationStage.Decoration.values())
        // try
        // {
        // int i = 0;
        //
        // for (final ConfiguredFeature<?, ?> feature :
        // biome.getFeatures(stage))
        // {
        ...
        // CrashReport.makeCrashReport(exception, "Feature placement");
        // crashreport.makeCategory("Feature").addDetail("Id",
        // ForgeRegistries.FEATURES.getKey(
            ...
            throws CommandSyntaxException
    {
        ...
        final Structure<?> structure = ForgeRegistries.STRUCTURE_FEATURES.getValue(key);

        // final ChunkGenerator generator =
        // worldIn.getChunkProvider().generator;
        // ChunkPos chunkpos = chunk.getPos();
        // final StructureStart<?> start =
        // structure.getStartFactory().create(structure, chunkpos.x,
        // chunkpos.z,
        ...
        // final List<IChunk> primers = Lists.newArrayList();
        // final List<Chunk> originals = Lists.newArrayList();
        // start.init(generator, templateManager, chunkpos.x, chunkpos.z,
        // biome);
        // final SharedSeedRandom rand = new
        // SharedSeedRandom(generator.getSeed());
        // rand.setLargeFeatureSeed(generator.getSeed(), chunkpos.x,
        // chunkpos.z);
        ...
        // {
        // final ChunkPos pos = new ChunkPos(x, z);
        // final ChunkPrimer chunk2 = new ChunkPrimer(pos,
        // UpgradeData.EMPTY);
        // primers_for_starts.add(chunk2);
        // all_primers.put(pos, chunk2);
        // if (x < mx - ds || x > mx + ds) continue;
        ...
        // worldRegion, box.minY, box.maxY + 1, true);
        // source.sendFeedback(new StringTextComponent("Generating Requested
        // Structure"), false);
        // final LazyOptional<IHasStructures> structs =
        // worldIn.getCapability(WorldStructures.CAPABILITY);
        ...
        // });
        // start.generateStructure(worldRegion, generator, rand, box,
        // chunkpos);
        // source.sendFeedback(new StringTextComponent("Done"), false);
        return 0;
    }
