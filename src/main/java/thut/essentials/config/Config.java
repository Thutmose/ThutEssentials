package thut.essentials.config;
...
public class Config
{
    public static abstract class ConfigData implements IConfigHolder
    {
        ...
        {
            Essentials.LOGGER.debug("{} config belongs to us!", configEvent.getConfig().getFileName());
        ...
        {
            Essentials.LOGGER.info("Loaded {} config file {}", this.MODID, configEvent.getConfig().getFileName());
            this.read(configEvent.getConfig());
        }
        ...
        {
            Essentials.LOGGER.info("Reading {}", config.getFileName());
            ...
            for (final Field f : values.keySet())
                try
                {
                    ...
                    Essentials.LOGGER.info("Set {} to {}", f.getName(), o);
                    f.set(this, o);
                    changed = true;
                }
                catch (final Exception e)
                {
                    Essentials.LOGGER.error("Error updating config value for " + f, e);
                }
            return changed;
        }
        ...
        private boolean write(final ModConfig config, final Map<Field, ConfigValue<?>> values)
        {
            ...
            for (final Field f : values.keySet())
                ...
                {
                    Essentials.LOGGER.error("Error saving config value for " + f, e);
                }
            return ret;
        }
    }
            ...
            final Type type)
    {
        ...
        for (final Field field : fields)
            try
            {
                ...
                builder.translation(ModLoadingContext.get().getActiveNamespace() + ".config." + field
                        .getName()
                        + ".tooltip");
    ...
    {
        Essentials.LOGGER.debug("Loading config file {}", path);
