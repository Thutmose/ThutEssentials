package thut.essentials.xml.bind;
...
import thut.essentials.xml.bind.annotation.XmlAttribute;
...
public class Factory<T>
{
    ...
    public static String trim(final String name)
    {
        ...
        if (Factory.trimmed.containsKey(name)) return Factory.trimmed.get(name);
        String trim = name;
    ...
    private void processNode(final Node node, final Object obj, final int depth)
    {
        ...
        if (children != null && children.getLength() > 0) for (int i = 0; i < children.getLength(); i++)
        {
            ...
            if (name.equals("#text"))
            {
                ...
                if (Factory.trim(value).isEmpty()) continue;
                // Else we try to set the corresponding object as this.
                Essentials.LOGGER.error("We do not handle this properly yet!");
                continue;
            }
